...
class ChromaDB(VectorDB):
    ...
    def add(self, embeddings, documents):
        ...
        for i, (emb, doc) in enumerate(zip(embeddings, documents)):
            if emb is not None and isinstance(emb, list) and all(isinstance(x, (float, int)) for x in emb) and doc and isinstance(doc, str):
        ...
        try:
            self.collection.add(ids=clean_ids, embeddings=clean_embeddings, documents=clean_documents)
            print("self.collection.count():", self.collection.count())
        except Exception as e:
            if "capture() takes 1 positional argument but 3 were given" in str(e):
                print("[ChromaDB.add] Telemetry bug encountered, but data likely added. Continuing...")
            else:
                print("[ChromaDB.add] Failed to add to collection:", e)
                import traceback; traceback.print_exc()
                print(f"ids type: {type(clean_ids)}, embeddings type: {type(clean_embeddings)}, documents type: {type(clean_documents)}")
